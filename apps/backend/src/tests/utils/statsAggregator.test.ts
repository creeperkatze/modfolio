...
import { aggregateProjectStats, aggregateProjectTypes, aggregateGameVersions, aggregateLoaders, aggregateCategories, findRecentProject, normalizeV3ProjectFields, aggregateAllStats } from "../../utils/statsAggregator.js";
...
describe("aggregateProjectStats", () => {
    it("sums downloads and followers", () => {
        const projects = [makeProject({ downloads: 1000, followers: 10 }), makeProject({ downloads: 2000, followers: 20 })];
    ...
    it("returns top projects sorted by downloads", () => {
        const projects = [makeProject({ downloads: 100 }), makeProject({ downloads: 500 }), makeProject({ downloads: 300 })];
...
describe("aggregateProjectTypes", () => {
    it("counts project types", () => {
        const projects = [makeProject({ project_type: "mod" }), makeProject({ project_type: "mod" }), makeProject({ project_type: "modpack" })];
...
describe("aggregateGameVersions", () => {
    it("returns top versions by frequency", () => {
        const projects = [makeProject({ game_versions: ["1.20", "1.19"] }), makeProject({ game_versions: ["1.20", "1.18"] }), makeProject({ game_versions: ["1.20"] })];
...
describe("aggregateLoaders", () => {
    it("counts loaders across projects", () => {
        const projects = [makeProject({ loaders: ["fabric", "quilt"] }), makeProject({ loaders: ["fabric"] })];
...
describe("aggregateCategories", () => {
    it("returns top categories by frequency", () => {
        const projects = [makeProject({ categories: ["utility", "adventure"] }), makeProject({ categories: ["utility"] })];
...
describe("findRecentProject", () => {
    it("returns most recently published project", () => {
        const projects = [makeProject({ published: "2024-01-01", title: "old" }), makeProject({ published: "2024-06-01", title: "new" }), makeProject({ published: "2024-03-01", title: "mid" })];
        expect(findRecentProject(projects).title).toBe("new");
    });

    it("ignores projects without published date", () => {
        const projects = [makeProject({ title: "no-date" }), makeProject({ published: "2024-01-01", title: "dated" })];
