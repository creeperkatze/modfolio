...
export function calculateBottomDelay(itemCount) {
	...
	const firstItemDelay =
		ANIMATION_DELAYS.SECTION_HEADER_DELAY + ANIMATION_DELAYS.SECTION_HEADER_TO_FIRST_ITEM_DELAY
	return (
		firstItemDelay + itemCount * ANIMATION_DELAYS.ITEM_DELAY + ANIMATION_DELAYS.BOTTOM_INFO_DELAY
	)
}
...
export function generateHeader(
	...
	colors,
	platformIcon = null,
	platformIconViewBox = '0 0 512 514',
) {
...
export function generateRectImage(
	imageUrl,
	clipId,
	x,
	y,
...
export function generateProjectListItem(
	project,
	index,
	totalDownloads,
	colors,
	...
	const { path: projectSparklinePath, fillPath: projectSparklineFillPath } = generateSparkline(
		projectVersionDates,
		sparklineWidth,
		sparklineHeight,
	)
...
${
    ...
    ${
			projectIconUrl
				? `<image x="20" y="${yPos - 12}" width="28" height="28" href="${projectIconUrl}" clip-path="url(#project-icon-clip-${index})"/>`
				: `<svg x="20" y="${yPos - 12}" width="28" height="28" viewBox="0 0 24 24">
...
export function generateProjectList(
	...
	const projectsHtml = topProjects
		.map((project, index) =>
...
export function generateVersionListItem(
	version,
	index,
	colors,
	relativeTime,
	...
	const gameVersionsText =
		gameVersions.slice(0, 3).join(', ') + (gameVersions.length > 3 ? '...' : '')
