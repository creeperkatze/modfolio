...
export function generateProjectCard(data, options, platformConfig, entityType = 'project') {
	...
	const latestVersions = showVersions
		? versions.slice(0, maxVersions).map((v) => ({
				...v,
				version_number: v.version_number || v.name || v.version || 'Unknown',
				date_published:
					v.date_published || v.releasedAt || v.createdAt || v.fileDate || new Date().toISOString(),
	...
	const height = hasVersions
		? 150 + latestVersions.length * 50 + summaryOffset
		: summaryOffset > 0
			? 130 + summaryOffset
			: 130
